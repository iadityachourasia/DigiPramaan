---
name: ux4g-design
description: Apply the UX4G Design System contract when designing, implementing, reviewing, or fixing government digital interfaces. Use for UX4G components, classes, tokens, themes, responsive layouts, accessibility, Figma parity, package or documentation decisions, page composition and layout, visual QA and screenshot review, and MCP/tooling setup for this project. Also use when starting a new build, planning a build sequence, or asking what to build next.
---
...
This skill's full knowledge base is split across supporting files in this same folder so each one loads only when needed, not on every session start. Read the relevant one(s) before acting:

- **`DESIGN_SYSTEM.md`** — tokens, colors, spacing, typography, the full UX4G contract merged with visual-composition principles. Read before any styling decision.
- **`COMPONENT_SPEC.md`** — the UX4G component parity table and composed recipes (MetricCard, ApplicationTracker, DataTable, etc.). Read before building any UI piece.
- **`PAGE_COMPOSITION.md`** — grid math and per-page-archetype layout specs. Read before laying out any new page.
- **`ACCESSIBILITY_AND_QA.md`** — the WCAG contract, known-failing tokens, and the 5-round audit checklist. Read before shipping any page.
- **`VISUAL_QA_LOOP.md`** — the screenshot-and-score loop. Read before presenting any page as finished.
- **`MCP_AND_TOOLING_SETUP.md`** — which MCP servers/skills to connect and which to avoid for this project. Read once at project start, and whenever considering adding a new tool.
- **`IMPLEMENTATION_GUIDE.md`** — the actual build sequence (BRD gate, preflight, phases). Read this first, in full, when starting a new build or asked what to build next.

## Mandatory preflight — do not write code yet

Before creating or changing any UX4G interface:

1. Read `docs/Design.md` completely (the raw UX4G contract this skill's `DESIGN_SYSTEM.md` is built from).
