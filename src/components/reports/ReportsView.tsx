...
export function ReportsView({ locale }: { locale: string }) {
  ...
  async function handleDownload(format: ReportFormat) {
    ...
    const href = await getRecordReportDownloadHref(
      builder.detail.report,
      format,
      user?.id
    );
  ...
  const completedReportId =
    builder.run?.status === "completed" ? builder.run.report?.id : null;
  ...
  return (
    <div className="lmcs-page-section">
      {/*
        Bounded to a single, document-width column (PAGE_COMPOSITION.md §4) —
        this is a form/report-composition flow, not a full-width data
        surface. Download History below stays outside this wrapper and runs
        the page's full width, per the same spec's "a data table is full
        width below the panel row".
      */}
      <div className="lmcs-report-builder">
        <ReportScopePanel
          scope={builder.scope}
      ...
      */}
        ...
        {builder.run ? (
          <section
            ...
            className="lmcs-page-section-block"
          >
        ...
        {builder.detail ? (
          <>
            ...
            <section
              aria-labelledby="report-download-heading"
              className="lmcs-page-section-block"
            >
